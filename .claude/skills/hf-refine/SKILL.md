...
9. 지원 언어 — 1차(`card_data.language`) 실패시 2차가 판단. **정책: 인코더 계열(ELECTRA/BERT 등 `pipeline_tag`가 없거나 encoder형 `model_type`)은 README에 명시된 언어만 엄격히 기재. 그 외 일반 대화형/생성형 LLM은 README에 "○○어 전용"처럼 명시적 제한이 없는 한 기본적으로 영어도 포함한다** (범용 생성형 LLM이 영어를 아예 못 다룰 가능성은 사실상 없기 때문 — 단, 그 외 언어(한국어 이외)는 여전히 명시적 근거가 있을 때만 추가). 통제 어휘: 한국어/영어/일본어/중국어/다국어(N개 언어)/기타(구체 언어명 병기), 복수는 항상 이 순서로 콤마 나열.
    - **(2026-07-20 변경) "다국어"로 뭉뚱그려도 4대 언어(한국어/영어/일본어/중국어) 포함 여부는 반드시 이름으로 남긴다** — 엑셀에서 "한국어 지원 모델만 필터링" 같은 검색이 가능해야 하기 때문에, 언어가 3개 이상이라고 "다국어(N개 언어)" 하나로 뭉개버리면 안 된다. 소스(구조화 필드 `card_data.language`의 언어 코드 목록, 또는 README가 개별 언어명을 나열한 경우)에서 4대 언어 중 확인되는 것들을 먼저 이름으로 나열하고, 그 뒤에 전체 개수를 `, 다국어(N개 언어)`로 덧붙인다 — 예: `card_data.language`가 `["ko","en","ja","zh","fr","de",...]`(총 32개)이면 `"한국어, 영어, 일본어, 중국어, 다국어(32개 언어)"`. **소스가 개별 언어명 없이 총 개수만 제공하는 경우(예: README에 "지원 언어: 102개"라고만 적혀 있고 목록이 없는 경우)에는 4대 언어 포함 여부를 알 수 없으므로 억지로 채우지 말고 그대로 `"다국어(N개 언어)"`만 쓴다** — 다른 모델(예: 이 모델이 쓰는 베이스/인코더)의 카드에서 목록을 가져와 채우는 것도 금지(다른 문서의 값을 이 모델 값인 것처럼 섞으면 안 됨).
10. 라이선스 **(필수)** — 1차(`card_data.license`) 실패시 2차(README 명시 문구만). 통제 어휘: MIT/Apache-2.0/CC-BY-4.0/CC-BY-NC-4.0/CC-BY-SA-4.0/OpenRAIL/Llama 커스텀 라이선스/기타(원문 이름 병기). **(2026-07-20 명확화) "원문 이름"은 `card_data.license` 슬러그(예: `k-exaone`, `upstage-solar-license`)를 그대로 괄호에 넣으라는 뜻이 아니다** — 슬러그는 사람이 읽기 어려우므로, README/모델카드에 적힌 실제 라이선스 명칭(예: "K-EXAONE AI Model License Agreement", "Upstage Solar License")을 찾아 그걸 병기한다. README에 별도 이름이 없어 슬러그 외엔 확인할 길이 없을 때만 슬러그를 그대로 쓴다.
...
**문장 표현 다양화 (2026-07-20 추가, 07-20 재점검 후 강화)**: "~에 적합" 한 가지 어미로 매번 끝맺지 않는다 — "~에 강점", "~용도로 최적", "~할 때 유용", "~에 특화" 등 문서의 실제 어조/내용에 맞춰 표현을 바꿔 쓴다. 같은 회사·같은 배치에서 여러 모델을 연속으로 작성할 때, 직전에 쓴 문장과 어미·문장 구조가 반복되면 다른 표현으로 바꾼다.
- **형식 규칙(강제): 완결된 서술형 문장("~하다.", "~합니다.", "~가능하다." 등 동사+마침표로 끝나는 종결형)으로 쓰지 않는다.** 항상 명사구/구절로 끝맺는 축약형("~에 적합", "~에 강점", "~에 특화", "~용도로 최적")만 쓴다 — `FriendliAI/c4ai-command-r-plus-fp8`에 "...에이전트형 업무에 적합하다."처럼 완결형 문장이 남아있던 사례를 실제로 발견해 추가한 규칙이다. 완결형 종결(마침표+동사)이 남아있으면 그 자체가 규칙 위반이니 발견 즉시 축약형으로 고친다.
- **자체 점검**: 설명을 쓴 뒤 스스로 다시 읽고 (1) 마침표로 끝나는 완결형 문장이 아닌지, (2) 같은 배치에서 방금 쓴 다른 설명과 어미가 겹치지 않는지 확인한다.
